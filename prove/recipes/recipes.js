...
// I used ChatGPT to figure out how to handle the "half star" ratings that exist //
...
>  Rating: `
    for (let i = 1; i <= 5; i++) {
      if (i <= Math.floor(rating)) {
        ...
        html += `<span aria-hidden="true">⯨</span>` //portion of the function that handles half stars
      } else {
...
/* we take the apple crisp recipe and paste it here and change the values
for example, the h2 will be a template literal value from the recipe array
up in the JavaScript file. */

/*tag template is a call to another function */
